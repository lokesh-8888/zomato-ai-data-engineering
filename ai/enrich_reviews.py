...
# Load the environment file stored alongside this script, regardless of the
...
if not openrouter_api_key:
    raise RuntimeError(
        "OPENROUTER_API_KEY is not set. Add it to ai/.env before running this script."
    )

# OpenRouter exposes an OpenAI-compatible API, so the existing OpenAI SDK works
# after pointing it at OpenRouter and supplying an OpenRouter API key.
client = OpenAI(
